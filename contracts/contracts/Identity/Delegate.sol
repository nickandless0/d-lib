...
contract Delegate {
  ...
  modifier onlyOwner(address identity, address actor) {
    emit OwnerRequest(identity, actor, msg.sender);
    require (actor == identityOwner(identity), "Only the owner can execute");
    _;
  }
  ...
  function checkSignature(address identity, uint8 sigV, bytes32 sigR, bytes32 sigS, bytes32 hash) internal returns(address) {
    ...
    require(signer == identityOwner(identity), "Only the owner can execute");
    nonce[signer]++;
    return signer;
  }
